...
public class Role_DetailDAL extends Manager {
    ...
    public List<Role_Detail> searchRole_detailsByRole(int role_id, String end) {
        try {
            return convertToRole_details(executeQuery("SELECT [rd].[role_id], [rd].[staff_id], [rd].[entry_date], [rd].[salary] \n" +
                    "FROM (SELECT [staff_id], MAX([entry_date]) as entry_date\n" +
                    "\t\t\tFROM  [dbo].[role_detail] \n" +
                    "\t\t\tWHERE [entry_date] <= '" + end + "'\n" +
                    "\t\t\tGROUP BY [staff_id]) tb1 JOIN [dbo].[role_detail] rd on [tb1].[staff_id] = [rd].[staff_id] AND [tb1].[entry_date] = [rd].[entry_date]\n" +
